...
echo ""
echo "INFO: download the public Tensorflow python script"
cmd="wget https://github.com/keras-team/keras-io/raw/master/examples/vision/mnist_convnet.py -O mnist_convnet.py --quiet"
echo "INFO: cmd=$cmd"
`$cmd`

echo ""
echo "INFO: submit the python script"
cmd="./$DLICMD --exec-start tensorflow --rest-host $WMLA_REST_HOST --appName my-tf-app --workerDeviceType cpu  --numWorker 1 --model-main mnist_convnet.py"
echo "INFO: cmd=$cmd"
result=`$cmd`
echo $result
...
echo ""
echo "INFO: get training stderr and stdout"
