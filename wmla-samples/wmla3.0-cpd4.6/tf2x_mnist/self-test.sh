...
cmd="./$DLICMD --exec-start disttensorflow --rest-host $WMLA_REST_HOST --appName my-tf-app --numWorker 3 --workerDeviceType cpu --model-main main.py --model-dir ../tf2x_mnist -- --epochs 10 --no-cuda true"
echo "INFO: cmd=$cmd"
result=`$cmd`
echo $result
...
echo "INFO: cmd=$cmd"
result=`$cmd`
expect="expect trainoutlogs contains *Test loss:* and *Test accuracy:*"
if [[ "$result" == *"Test loss:"* ]] && [[ "$result" == *"Test accuracy:"* ]] ; then
  echo "INFO: $expect. OK"
else
  echo "ERROR: $expect. Got $result. FAILED"
fi

expect="expect trainoutlogs contains *TF_CONFIG is founded* "
if [[ "$result" == *"TF_CONFIG is founded"* ]] ; then
  echo "INFO: $expect. OK"
else
  echo "ERROR: $expect. Got $result. FAILED"
fi
