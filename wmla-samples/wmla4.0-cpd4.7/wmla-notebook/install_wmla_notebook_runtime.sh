...
wmla_guid=`curl -s -k -H "Authorization: Bearer ${myToken}" "https://${cpd_host}/v2/runtime_definitions" | jq -r '.resources[] |select(.entity.name=="jupyter-wmla-py") |.metadata.guid'`
if [ ! -z "${wmla_guid}" ]
then
...
ws_guid=`curl -s -k -H "Authorization: Bearer ${myToken}" "https://${cpd_host}/v2/runtime_definitions" | jq -r '.resources[] |select(.entity.name=="jupyter-231n-py") |.metadata.guid'`
...
curl -s -k -H "Authorization: Bearer ${myToken}" "https://${cpd_host}/v2/runtime_definitions/${ws_guid}?include=launch_configuration" | jq -r '.entity | .name="jupyter-wmla-py" | .launch_configuration.env += [{
      ...
      },
      {
        ...
        "subPath": "notebook",
        "optional": false
      }]' > jupyter-wmla-py-server.json
echo Save configurations to jupyter-wmla-py-server.json
echo -e "Create WMLA notebook runtime configurations - ${SUCCESS}"

echo
echo Upload WMLA notebook runtime configurations...
output=`curl -k -s -X POST -H "Authorization: Bearer ${myToken}" "https://${cpd_host}/v2/runtime_definitions/" -H 'Content-Type: application/json' -d @jupyter-wmla-py-server.json`
echo ${output}
