...
def train(args):
    ...
    # Check if the variables are stored on GPU
    if all('gpu' in weight.device.lower() for weight in model.weights):
        print("Model parameters are on cuda")
    else:
        print("Model parameters are on cpu")
