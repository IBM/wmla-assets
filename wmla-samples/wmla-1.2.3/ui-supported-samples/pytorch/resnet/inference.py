...
def inference(args):
    ...
    for imgFile in imagefiles:
       ...
       print (prob.data.numpy()[0])
       print (label.data.numpy()[0])
       
       prediction = np.append(prediction, [prob.data.numpy()[0]], 0)
       imagenames = np.append(imagenames, [imgFile], 0)
       labeldatas = np.append(labeldatas, [label.data.numpy()[0]], 0)
...
def validate(args):
    ...
    #model.fc = torch.nn.Linear(2048, 10)  #change to 10 output for cifar10
