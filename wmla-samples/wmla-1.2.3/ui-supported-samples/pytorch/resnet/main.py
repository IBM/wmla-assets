...
def main(model_type, pretrained = False):
    ...
    #pytorch_monitor = PytorchMonitor(model, optimizer, train_loader, test_loader, epochs, conf="monitor.yaml")
    ...
    for epoch in range(1, epochs+1):
        if scheduler and not isSchedulerReduceLROnPlateau:
            scheduler.step()
        #train(model, device, train_loader, optimizer, epoch, pytorch_monitor)
        #test(model, device, test_loader, epoch, pytorch_monitor)
